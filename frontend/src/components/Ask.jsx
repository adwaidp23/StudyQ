...
export default function Ask({ onNewQuestion }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem' }} className="animate-fade-in">
      <div className="glass-card">
        ...
        <p style={{ color: 'var(--text-secondary)', marginBottom: '1.5rem' }}>
          Type your question below. Our AI will automatically tag the topic and find semantically similar past questions.
        </p>
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button type="submit" className="btn-primary" disabled={loading || !text.trim()}>
              {loading ? <div className="spinner" /> : <><Search size={18} /> Find Similar</>}
            </button>
          </div>
          {error && <div style={{ color: 'var(--danger-text)', marginTop: '1rem', fontSize: '0.875rem' }}>{error}</div>}
        </form>
      </div>
        ...
        <div className="glass-card animate-fade-in" style={{ borderLeft: '4px solid var(--success-text)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', color: 'var(--success-text)', fontWeight: '600' }}>
              <CheckCircle2 size={20} /> Question Saved
            </div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', marginBottom: '1.5rem' }}>
            <span style={{ color: 'var(--text-secondary)', fontSize: '0.875rem' }}>Auto-tagged as:</span>
          ...
          <div style={{ paddingTop: '1.5rem', borderTop: '1px solid var(--border-subtle)' }}>
            <h4 style={{ color: 'var(--text-secondary)', textTransform: 'uppercase', letterSpacing: '0.05em', fontSize: '0.75rem', marginBottom: '1rem' }}>
              {result.similar.length > 0 ? `${result.similar.length} Similar Questions Found` : 'No similar questions found yet!'}
            </h4>
